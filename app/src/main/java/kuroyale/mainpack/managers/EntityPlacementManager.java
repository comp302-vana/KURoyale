...
public class EntityPlacementManager {
     ...
     */
    public void setNetworkBattleManager(kuroyale.mainpack.network.NetworkBattleManager battleManager, boolean isClient) {
        this.networkBattleManager = battleManager;
        this.isClient = isClient;
    }
    /**
     * Determine which player (1 or 2) based on drop location.
     * Player 1 controls left side (col < 15), Player 2 controls right side (col >= 16).
    ...
    private int determinePlayerFromDropLocation(int targetCol) {
        ...
        // Use column-based zone: left side (col < 15) = Player 1, right side (col >= 16) = Player 2
    ...
    private void handleClientCardDrop(DragEvent event, int cardID, int clientRow, int clientCol) {
        ...
        if (!CoordinateTransformer.isValidPlayer2Zone(absoluteRow, absoluteCol, rows, cols)) {
            System.out.println("Client: Invalid placement zone - requested absolute (" + absoluteRow + ", " + absoluteCol + 
        ...
        // - Cycle card locally (host will send ELIXIR_UPDATE, client cycles on ENTITY_SPAWN)
     ...
     * Called by NetworkBattleManager when CARD_PLACEMENT_REQUEST message is received.
     */
