...
public class ComboDetector {
     ...
     */
    public ComboType onCardPlayed(int cardID, AliveEntity playedEntity, int playerId) {
        ...
        CardPlayRecord record = new CardPlayRecord(cardID, currentTime, playedEntity, playerId);
    ...
    private void cleanOldCardPlays(long currentTime) {
        recentCardPlays.removeIf(record -> 
            (currentTime - record.timestamp) > COMBO_WINDOW_SECONDS * 1000
        );
    }
    ...
    private ComboType checkForCombos(CardPlayRecord newRecord) {
        for (CardPlayRecord oldRecord : recentCardPlays) {
            ...
            if (timeDiff > COMBO_WINDOW_SECONDS) continue;
    ...
    private boolean isSwarmCard(int cardID) {
        return cardID == 9 || cardID == 10 || cardID == 11 || cardID == 12 || 
               cardID == 13 || cardID == 14 || cardID == 15; // Skeletons, Goblins, Spear Goblins, Archers, Minions, Minion Horde, Barbarians
    }
    ...
    public static class CardPlayRecord {
        ...
        public CardPlayRecord(int cardID, long timestamp, AliveEntity entity, int playerId) {
            this.cardID = cardID;
