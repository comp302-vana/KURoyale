...
public class CardVisualManager {
    ...
    /**
     * Creates a level indicator node (stars or number).
    ...
    public static Node createLevelIndicator(int level) {
        ...
        // Create stars display: ★ for each level
        StringBuilder stars = new StringBuilder();
        for (int i = 0; i < level; i++) {
            stars.append("★");
        }
        levelLabel.setText(stars.toString());
    ...
    public static void updateLevelIndicator(AnchorPane cardContainer, int level) {
        ...
        // Remove existing level indicator (look for Label with stars)
        cardContainer.getChildren().removeIf(node -> 
            node instanceof Label && 
            ((Label) node).getText() != null && 
            ((Label) node).getText().contains("★")
        );
